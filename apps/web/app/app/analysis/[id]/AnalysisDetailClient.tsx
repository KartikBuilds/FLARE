        ...
        {tab === "report" && (
          <Card>
            ...
            <p className="mt-2 max-w-lg font-sans text-sm text-muted">
              A versioned JSON report with the full IR, findings and graph, and a self-contained,
              printable HTML report (findings, evidence, the FLARE score breakdown and a static
              fund-flow diagram — Save as PDF from your browser&apos;s print dialog for a PDF
              copy). A dedicated Tool Comparison / State Model / Dependencies view is still
              planned — see{" "}
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              ...
              {analysis.origin !== "demo" && getAnalyzerApiBase() ? (
                <ButtonLink
                  href={`${getAnalyzerApiBase()}/analyses/${analysis.id}/report.html`}
                  external
                  variant="outline"
                  arrow="none"
                >
                ...
                <p className="self-center font-sans text-xs text-muted">
                  {analysis.origin === "demo"
                    ? "The HTML report is generated by the real analyzer — demo analyses don't have one."
                    : "Set NEXT_PUBLIC_ANALYZER_API_URL to enable the HTML report link."}
                </p>
              )}
            </div>
          </Card>
